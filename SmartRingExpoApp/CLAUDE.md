...
- **No emoji in UI:** Never use emoji characters as icons or decorative elements in the UI. Always use `Ionicons` from `@expo/vector-icons/Ionicons` instead (e.g., `sunny-outline` for wake, `cafe-outline` for caffeine, `moon-outline` for sleep). Emoji are only acceptable inside user-generated or data-driven content (e.g., drink preset keys defined in `CAFFEINE_PRESETS`).
- **Custom SVG icons → `src/assets/icons/`:** When the user pastes an SVG (or a new custom SVG icon is needed), ALWAYS create a `.tsx` file in `src/assets/icons/` following the existing pattern (named export + default export, `size`/`color`/`width`/`height` props with sensible defaults, `Svg` + `Path`/etc from `react-native-svg`). Then add the export to `src/assets/icons/index.ts`. Import the icon from `'../../assets/icons'` (or the relative equivalent) — never define inline SVG component functions inside a screen or component file. Existing icons: `BedtimeIcon`, `SleptIcon`, `WakeTimeIcon`, `NightTimeIcon`, `SleepScoreIcon`, `SleepIcon`, `ActivityIcon`, `OverviewIcon`, `RingIcon`, `BandIcon`.
- **No background fills on metric/detail screens:** Components (cards, chart wrappers, suggestion tiles) must use `borderWidth` + `borderColor` only — no `backgroundColor`. Log/action buttons follow the same rule, with the sole exception being a **white background (`#FFFFFF`) + black text (`#000000`)** for primary CTA buttons on dark screens (e.g. the adenosine detail "Log drink" button).
...
- **"Cover" terminology:** The card/component on the overview tab that links to a detail page is called the **cover**. E.g. `CaffeineWindowCard` = adenosine cover, `DailyHeartRateCard` = HR cover. The full-screen page it navigates to is the **detail**. Use this naming in all future code, docs, and conversation.
