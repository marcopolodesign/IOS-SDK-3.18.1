...
interface HRTrendChartProps {
  ...
  /** restingHR per day; 0 = no data */
  hrValues: Array<{ dateKey: string; restingHR: number }>;
  selectedIndex: number;
  onSelectDay: (index: number) => void;
}

export function HRTrendChart({ dayEntries, hrValues, selectedIndex, onSelectDay }: HRTrendChartProps) {
  ...
  // Use raw resting HR values. Scale: 0–120 bpm range for bar heights.
  ...
  const rawValues = reversed.map(d => hrValues.find(s => s.dateKey === d.dateKey)?.restingHR ?? 0);
  ...
  reversed.forEach((_, i) => {
    ...
    if (avg !== null) {
      ...
      const barH = (Math.min(avg, MAX_DISPLAY) / MAX_DISPLAY) * maxBarH;
          ...
          <Svg width={contentW} height={CHART_H}>
            <G>
              {[30, 60, 90].map(threshold => {
                const lineY = baseline - (threshold / MAX_DISPLAY) * maxBarH;
                return (
                  <Line
                    key={threshold}
              ...
              {reversed.map((d, i) => {
                const v = rawValues[i];
                const barH = Math.max(3, (Math.min(v, MAX_DISPLAY) / MAX_DISPLAY) * maxBarH);
                const cx = i * COL_W + COL_W / 2;
                const x = cx - BAR_W / 2;
                ...
                return (
                  <G key={d.dateKey}>
                    <Path
                      ...
                      fill={isSel ? hrColor(v) : 'rgba(255,255,255,0.4)'}
                    />
                    {showLabel && (
                      <SvgText
                        x={cx}
