...
-(void)BleCommunicateWithPeripheral:(CBPeripheral*)Peripheral data:(NSData *)data
{
    ...
    if(deviceData.dataType == DetailSleepData_X3)
    {
        count +=1;
        ...
        {
            // 继续读取剩下的数据
            NSData * data = [[BleSDK_X3 sharedManager] GetDetailSleepDataWithMode:2 withStartDate:nil];
...
- (IBAction)getData:(UIButton *)sender {
    ...
    if([[NewBle sharedManager] isConnectOrConnecting]==YES){
            ...
            strText = @"";
        NSData * data = [[BleSDK_X3 sharedManager] GetDetailSleepDataWithMode:0 withStartDate:nil];
        [[NewBle sharedManager] writeValue:SERVICE characteristicUUID:SEND_CHAR p:[NewBle sharedManager].activityPeripheral data:data];
    }
    else
...
- (IBAction)deleteData:(UIButton *)sender {
    if([[NewBle sharedManager] isConnectOrConnecting]==YES){
        ...
        strText = @"";
        NSData * data = [[BleSDK_X3 sharedManager] GetDetailSleepDataWithMode:0x99 withStartDate:nil];
