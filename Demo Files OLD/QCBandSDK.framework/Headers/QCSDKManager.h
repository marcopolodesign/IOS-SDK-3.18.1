...
typedef NS_ENUM(NSInteger, QCMeasuringType) {
    ...
    QCMeasuringTypeHRV,
    QCMeasuringTypeBodyTemperature,     //blood temperature measurement
    QCMeasuringTypeThreeValueBodyTemperature,     //blood temperature measurement
    QCMeasuringTypeOneKeyMeasureHeartRate,     //Heart rate onekey measurement
    QCMeasuringTypeHeartRateRaw,     //Heart rate measurement raw data
    QCMeasuringTypeBloodOxygenRaw,     //oxygen measurement raw data
...
/**
 *  Report on wearing on both hands
 *  左右手佩戴上报
 *  flipWristInfo: enable:enable or not; hand:1-left,2-right
 */
@property(nonatomic,copy)void(^flipWristInfo)(NSInteger enable,NSInteger hand);

/**
 *  Gesture and touch information
 *  手势和触摸信息
 *  gestureAndTouchInfo: mode:0-touch,1-gesture;  type:QCTouchGestureControlType
 */
@property(nonatomic,copy)void(^gestureAndTouchInfo)(NSInteger mode,QCTouchGestureControlType type);

/**
 *  Touch Sleep information
 *  触摸休眠信息
 *  touchSleepInfo: sleeping:sleep or not
 */
@property(nonatomic,copy)void(^touchSleepInfo)(BOOL sleeping);
...
/// @param handle                 :Measurement result callback (error code: -1: failed to send start command, -2: failed to send end command, -3: bracelet is not properly worn,-4:Uncalibrated)
